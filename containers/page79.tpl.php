...
<body id="bodywebsite" class="bodywebsite bodywebpage-detailspropale">
<?php
...
// Récupérer et nettoyer l'ID de propal à partir de l'URL
$propal_id = intval($_GET['propal_id']);

// Récupérer les informations de la propal depuis la base de données
$sql_check_propal = "SELECT rowid, ref FROM " . MAIN_DB_PREFIX . "propal WHERE rowid = $propal_id";
$result_check_propal = $db->query($sql_check_propal);

if ($db->num_rows($result_check_propal) == 0) {
    echo "propal avec l'ID $propal_id introuvable";
    exit;
}

// Récupérer la référence de la propal
$propal = $db->fetch_object($result_check_propal);
$propal_ref = $propal->ref;

// Construire le chemin du fichier PDF
$pdf_path = $_SERVER['DOCUMENT_ROOT'] . '/documents/propale/' . $propal_ref . '/' . $propal_ref . '.pdf';
...
if (file_exists($pdf_path)) {
    ...
    echo "Le fichier PDF pour la propal $propal_ref n'existe pas.";
    exit;
}
?>
...
<?php // BEGIN PHP
$tmp = ob_get_contents(); ob_end_clean(); dolWebsiteOutput($tmp, "html", 79); dolWebsiteIncrementCounter(9, "page", 79);
// END PHP ?>
